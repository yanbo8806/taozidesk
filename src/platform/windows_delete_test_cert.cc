// https://github.com/taoziapp/taoziapp/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeletetaoziappTestCertsW_SingleHive()
//
//  Purpose:    Deletes taoziapp Test certificates and wrong key stores
//
...
BOOL DeletetaoziappTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeletetaoziappTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeletetaoziappTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeletetaoziappTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeletetaoziappTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
