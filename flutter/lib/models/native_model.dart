...
class PlatformFFI {
  ...
  taoziappImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libtaoziapp.so')
        : isLinux
            ? DynamicLibrary.open('libtaoziapp.so')
            : isWindows
                ? DynamicLibrary.open('libtaoziapp.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibtaoziapp.dylib") :
  ...
  void _startListenEvent(taoziappImpl taoziappImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = taoziappImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
