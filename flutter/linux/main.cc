...
#define taoziapp_LIB_PATH "libtaoziapp.so"
typedef bool (*taoziappCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_taoziapp_core_main() {
   void* libtaoziapp = dlopen(taoziapp_LIB_PATH, RTLD_LAZY);
   if (!libtaoziapp) {
      fprintf(stderr,"Failed to load \"libtaoziapp.so\"\n");
      char* error;
   ...
   auto core_main = (taoziappCoreMain) dlsym(libtaoziapp,"taoziapp_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"taoziapp_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
