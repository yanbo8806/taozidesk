...
typedef char** (*FUNC_taoziapp_CORE_MAIN)(int*);
typedef void (*FUNC_taoziapp_FREE_ARGS)( char**, int);
typedef int (*FUNC_taoziapp_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libtaoziapp.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libtaoziapp.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_taoziapp_CORE_MAIN taoziapp_core_main =
      (FUNC_taoziapp_CORE_MAIN)GetProcAddress(hInstance, "taoziapp_core_main_args");
  if (!taoziapp_core_main)
  {
    std::cout << "Failed to get taoziapp_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_taoziapp_FREE_ARGS free_c_args =
      (FUNC_taoziapp_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = taoziapp_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "taoziapp [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"taoziapp";
  FUNC_taoziapp_GET_APP_NAME get_taoziapp_app_name = (FUNC_taoziapp_GET_APP_NAME)GetProcAddress(hInstance, "get_taoziapp_app_name");
  if (get_taoziapp_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_taoziapp_app_name(app_name_buffer, 512) == 0) {
